...
class AttentionLayer(nn.Module):
	...
	def __init__(self, query_size, key_size, value_size=None, mode='bahdanau',
		...
		# define var
		value_size = value_size or key_size  # Usually key, values are the same
		self.mode = mode
	...
	def forward(self, query, keys, values=None, prev_c=None, use_gpu=True):
		...
		# b x t_q x t_k
		scores, c_out = self.calc_score(att_query, keys, prev_c, use_gpu=use_gpu)
		...
		if hasattr(self, 'hard_att'):
			if self.hard_att:
				...
				scores_normalized = (scores_hard / total_score).to(device=device)
			else:
